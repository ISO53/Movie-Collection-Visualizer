...
async function clearDatabase() {
  if (confirm('Are you sure you want to clear the entire database? This will remove all movies and reset all settings (EXCEPT your API key). This cannot be undone.')) {
    try {
    ...
    <div class="settings-card">
      ...
      <div class="card-body">
        <div class="input-group">
          ...
          <button class="action-btn primary" @click="saveKey" :disabled="!localKey.trim()">
            Save Key
          </button>
        </div>
        ...
        <div v-if="settingsStore.watchedDirectory" class="directory-display">
          ...
          <div class="button-row">
            ...
            <div class="button-group">
              ...
              <button class="action-btn outline danger" @click="clearDir">
                <X :size="16" /> Remove
              </button>
            </div>
          </div>
    ...
    <div class="settings-row">
      ...
      <div class="settings-card about-card">
        ...
        <div class="card-body">
          ...
          <button class="action-btn outline full-width mt-4" @click="openUrl('https://github.com/ISO53/Movie-Collection-Visualizer')">
        ...
        <div class="card-body">
          <div v-if="updateInfo" class="update-status">
            ...
            <div v-else class="new-update-available">
              ...
              <button class="action-btn primary mt-2" @click="openUrl(updateInfo.url)">Download Update</button>
            </div>
          </div>
    ...
    <div class="settings-card danger-card">
      ...
      <div class="card-body">
        <div class="danger-content">
          <p>Reset the application database. This will remove all indexed movies and history. Your API key will be preserved.</p>
          <button class="action-btn danger-solid" @click="clearDatabase">
...
.icon-wrapper.primary { background: rgba(236, 130, 0, 0.1); color: var(--accent-four); }
.icon-wrapper.secondary { background: rgba(229, 186, 115, 0.1); color: var(--accent-three); }
.icon-wrapper.ghost { background: rgba(255, 255, 255, 0.05); color: var(--muted-mid); }
.icon-wrapper.danger-bg { background: rgba(239, 68, 68, 0.1); color: #ef4444; }
...
.info-item .label { color: var(--muted-mid); font-size: 13px; }
.info-item .value { color: var(--text-main); font-size: 13px; font-weight: 600; }
