...
async function exportPdf() {
    ...
    doc.setFontSize(12);
    doc.text(
        `Total Movies: ${totalMovies.value}  |  Average Rating: ${averageRating.value}  |  Total Runtime: ${totalRuntime.value}`,
        14,
        32,
    );
        ...
        <div class="charts-row three-col">
            ...
            <div class="chart-card">
                <div class="card-header-actions">
                    ...
                    <button
                        ...
                        @click="directorSort = directorSort === 'count' ? 'rating' : 'count'"
                    ...
                    <div v-for="d in lollipopDirectors" :key="d.name" class="lollipop-item" @click="goToPerson(d.name)">
                        <div class="loll-track">
                            <div
                                class="loll-bar"
                                :style="{
                                    width: (d.count / Math.max(...lollipopDirectors.map((x) => x.count))) * 90 + '%',
                                }"
                            >
                                <span class="loll-name-in"
                                    >{{ d.name }} <span class="loll-count-in">({{ d.count }})</span></span
                                >
                            </div>
                            <div
                                class="loll-dot"
                                :style="{
                                    backgroundColor: getRatingColor(d.avgRating),
                                    left: (d.count / Math.max(...lollipopDirectors.map((x) => x.count))) * 90 + '%',
                                }"
                            >
            ...
            <div class="chart-card">
                <div class="card-header-actions">
                    ...
                    <button
                        class="sort-toggle-btn"
                        @click="actorSort = actorSort === 'count' ? 'rating' : 'count'"
                ...
                <div class="lollipop-container">
                    <div v-for="a in lollipopActors" :key="a.name" class="lollipop-item" @click="goToPerson(a.name)">
                        <div class="loll-track">
                            ...
                            >
                                <span class="loll-name-in"
                                    >{{ a.name }} <span class="loll-count-in">({{ a.count }})</span></span
                                >
                            </div>
                            <div
                                class="loll-dot"
                                :style="{
                                    backgroundColor: getRatingColor(a.avgRating),
                                    left: (a.count / Math.max(...lollipopActors.map((x) => x.count))) * 90 + '%',
                                }"
                            >
            ...
            <div class="chart-card">
                <div class="card-header-actions">
                    ...
                    <button
                        class="sort-toggle-btn"
                        @click="writerSort = writerSort === 'count' ? 'rating' : 'count'"
                    ...
                    <div v-for="w in lollipopWriters" :key="w.name" class="lollipop-item" @click="goToPerson(w.name)">
                        <div class="loll-track">
                            <div
                                class="loll-bar"
                                :style="{
                                    width: (w.count / Math.max(...lollipopWriters.map((x) => x.count))) * 90 + '%',
                                }"
                            >
                                <span class="loll-name-in"
                                    >{{ w.name }} <span class="loll-count-in">({{ w.count }})</span></span
                                >
                            </div>
                            <div
                                class="loll-dot"
                                :style="{
                                    backgroundColor: getRatingColor(w.avgRating),
                                    left: (w.count / Math.max(...lollipopWriters.map((x) => x.count))) * 90 + '%',
                                }"
                            >
